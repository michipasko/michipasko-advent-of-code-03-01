...
const transformArray = (wirepath, transformToArray) => {
    ...
    for (let untransformedCoordinate of wirePathArray) {
        ...
        else if (untransformedCoordinate[0] === "U") {
            let distance = parseInt(untransformedCoordinate.substring(1,))
            for (let i = 1; i <= distance; i++) {
                coordinateToPush[1] ++
                wirePathArrayTransformed.push(JSON.stringify(coordinateToPush))
        ...
        else if (untransformedCoordinate[0] === "D") {
            let distance = parseInt(untransformedCoordinate.substring(1,))
            for (let i = 1; i <= distance; i++) {
                coordinateToPush[1] --
                wirePathArrayTransformed.push(JSON.stringify(coordinateToPush))
        ...
        coordinate = coordinateToPush
        
    }
    return wirePathArrayTransformed
}


// deletes duplicates from transformed arrays
const deleteDuplicatesFromArray = (wirePathString) => {
    let wirePathArrayTransformed = transformArray(wirePathString, transformToArray)
    wirePathArrayTransformed = new Set(wirePathArrayTransformed)
    let wirePathArrayWithoutDuplicates = [...wirePathArrayTransformed]
    return wirePathArrayWithoutDuplicates
}


// Concatenate arrays and sort the new array
const concatenateAndSort = (wirePathString1, wirePathString2) => {
    let wirePathArrayTransformed1 = deleteDuplicatesFromArray(wirePathString1)
    let wirePathArrayTransformed2 = deleteDuplicatesFromArray(wirePathString2)
    let concatenatedWirePath = wirePathArrayTransformed1.concat(wirePathArrayTransformed2)
    concatenatedWirePath.sort()
    return(concatenatedWirePath)
}


// Get all intersections
const getAllIntersections = (wirePathString1, wirePathString2) => {
    const concatenatedWirePath = concatenateAndSort(wirePathString1, wirePathString2)

    const intersections = []

    let itemBefore = "" 
    for (let e of concatenatedWirePath) {
        if (e === itemBefore) {
...
const getShortestDistance = (wirePathString1, wirePathString2) => {
    const intersections = getAllIntersections(wirePathString1, wirePathString2)
    let distanceArray = []
    ...
    for (let intersection of intersections) {
        ...
        let distance = Math.abs(parseInt(intersection[0])) + Math.abs(parseInt(intersection[1]))
...
// Solution
const shorstestDistance = getShortestDistance(wirePathString1, wirePathString2)
console.log("Solution Task 1:", shorstestDistance)


// Testing
const wirePathTest1_1 = "R8,U5,L5,D3"
const wirePathTest1_2 = "U7,R6,D4,L4"
console.log("Test 1:", getShortestDistance(wirePathTest1_1, wirePathTest1_2) === 6)

const wirePathTest2_1 = "R75,D30,R83,U83,L12,D49,R71,U7,L72"
const wirePathTest2_2 = "U62,R66,U55,R34,D71,R55,D58,R83"
console.log("Test2:", getShortestDistance(wirePathTest2_1, wirePathTest2_2) === 159)

const wirePathTest3_1 = "R98,U47,R26,D63,R33,U87,L62,D20,R33,U53,R51"
const wirePathTest3_2 = "U98,R91,D20,R16,D67,R40,U7,R15,U6,R7"
console.log("Test 3:", getShortestDistance(wirePathTest3_1, wirePathTest3_2) === 135)
